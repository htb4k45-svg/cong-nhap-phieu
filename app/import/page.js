...
function parseMTSheet(ws) {
  ...
  for (let i = 0; i < rows.length; i++) {
    if (rows[i].some(c => String(c || '').toUpperCase().includes('SỐ PHIẾU'))) { headerIdx = i; break; }
  }
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    const ngayCan  = parseDate(ngayDiIdx >= 0 && row[ngayDiIdx] ? row[ngayDiIdx] : ngayYcIdx >= 0 ? row[ngayYcIdx] : null);
    ...
    phieuList.push({
      bo_phan: 'MT', ma_lenh: maLenh ? String(maLenh).trim() : null,
      so_phieu: soPhieu ? String(soPhieu).trim() : (maLenh ? String(maLenh).trim() : ''),
      ngay_nhap: ngayNhap, ma_kh: mstIdx >= 0 && row[mstIdx] ? String(row[mstIdx]).trim() : null,
      ...
      ngay_can_giao: ngayCan, ma_kho: khoVal, ten_kho: khoVal,
      ...
      san_pham: [], dac_diem: parseDacDiem(loaiXuatVal, null), ghi_chu: ghiChu || null,
    });
  }
  return phieuList;
}
...
function parseB2BSheet(ws) {
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    const khoVal = khoIdx >= 0 && row[khoIdx] ? String(row[khoIdx]).trim() : null;
    phieuList.push({
      bo_phan: 'B2B', so_phieu: String(soPhieu).trim(), ngay_nhap: ngayNhap,
      ten_kh: khachIdx >= 0 && row[khachIdx] ? String(row[khachIdx]).trim() : '',
      dia_chi_giao: diaChiIdx >= 0 && row[diaChiIdx] ? String(row[diaChiIdx]).trim() : '',
      ngay_can_giao: ngayCan, ma_kho: khoVal, ten_kho: khoVal,
      lai_xe: laiXeIdx >= 0 && row[laiXeIdx] ? String(row[laiXeIdx]).trim() : null,
      giao_nhan: giaoNhanIdx >= 0 && row[giaoNhanIdx] ? String(row[giaoNhanIdx]).trim() : null,
      san_pham: sanPham, dac_diem: parseDacDiem(loaiXuatVal, row[diaChiIdx]),
    });
  }
  return phieuList;
}
...
export default function ImportPage() {
  ...
  const totalSelected = sheets.filter(s => selected.includes(s.name)).reduce((sum, s) => sum + s.phieu.length, 0);

  const handleImport = async () => {
    const allPhieu = sheets.filter(s => selected.includes(s.name)).flatMap(s => s.phieu);
    setImporting(true);
    try {
