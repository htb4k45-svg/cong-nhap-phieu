...
// Map province norm → tên hiển thị (dùng trước ward-matcher để tránh match nhầm)
const PROVINCE_MAP_UI = [
  ['ha noi',       'Hà Nội'],   ['ho chi minh', 'HCM'],      ['tphcm',      'HCM'],
  ['hai phong',    'Hải Phòng'],['da nang',      'Đà Nẵng'],  ['quang ninh', 'Quảng Ninh'],
  ['hai duong',    'Hải Dương'],['hung yen',     'Hưng Yên'], ['bac ninh',   'Bắc Ninh'],
  ['bac giang',    'Bắc Giang'],['thai nguyen',  'Thái Nguyên'],['vinh phuc','Vĩnh Phúc'],
  ['phu tho',      'Phú Thọ'],  ['nam dinh',     'Nam Định'], ['ninh binh',  'Ninh Bình'],
  ['thanh hoa',    'Thanh Hóa'],['nghe an',      'Nghệ An'],  ['ha tinh',    'Hà Tĩnh'],
  ['binh duong',   'Bình Dương'],['dong nai',    'Đồng Nai'], ['long an',    'Long An'],
  ['tp thu duc',   'TP Thủ Đức'],['thu duc',     'TP Thủ Đức'],
];

function matchProvinceUI(normAddr) {
  for (const [key, name] of PROVINCE_MAP_UI) {
...
function getTongThung(p) {
  // Ưu tiên tong_thung đã tính sẵn từ parser (đã là A4-equiv từ sheets.js)
  if (p.tong_thung !== undefined && p.tong_thung !== null) return p.tong_thung;
  // Fallback: tính từ san_pham với hệ số A4-equiv
  if (p.san_pham && p.san_pham.length) {
    const raw = p.san_pham.reduce(function(acc, sp) {
      const thung = sp.so_luong_thung !== undefined ? sp.so_luong_thung : (sp.so_luong || 0);
...
export default function DieuXePage() {
  ...
  const [suggResult, setSuggResult]   = useState(null);   // kết quả gợi ý phân xe
  ...
  const applySuggestion = useCallback(async () => {
    ...
    setSuggApplying(true);
    try {
      const tasks = [];
      for (const asgn of suggResult.assignments) {
        if (!asgn.orders.length) continue;
        for (const order of asgn.orders) {
          tasks.push(fetch('/api/dispatch-status', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              row_key:          order.row_key,
              bo_phan:          order.bo_phan,
              ngay_giao:        order.ngay_can_giao || ngayTu,
              lai_xe_phan_cong: asgn.driver.ten,
            }),
          }));
        }
      }
      ...
      setSuggModal(false);
      setSuggResult(null);
      showToast('Đã áp dụng gợi ý phân xe!', 'ok');
    } catch (e) {
      showToast('Lỗi áp dụng: ' + e.message, 'err');
    } finally {
      setSuggApplying(false);
    }
  }, [suggResult, ngayTu, showToast]);

  // ── Export Excel gợi ý phân xe ────────────────────────────────────────────
  const exportSuggExcel = useCallback(() => {
    if (!suggResult || !suggResult.assignments) return;
    const today = new Date().toLocaleDateString('vi-VN');
    const rows = [];

    // Header tiêu đề
    rows.push(['PHÂN CÔNG XE GIAO HÀNG', '', '', '', '', '', '', '', today, '']);
    rows.push([`Tổng: ${suggResult.meta?.total || 0} đơn — ${suggResult.meta?.assigned || 0} đã gán — ${suggResult.meta?.drivers || 0} xe`, '', '', '', '', '', '', '', '', '']);
    rows.push([]);
    rows.push(['STT', 'Lái xe', 'Biển số', 'Khu vực', 'Số phiếu', 'Khách hàng', 'Địa chỉ giao', 'Thùng (A4)', 'Ngày cần giao', 'Bộ phận']);

    let stt = 1;
    for (const asgn of suggResult.assignments) {
      if (!asgn.orders.length) continue;
      for (const o of asgn.orders) {
        rows.push([
          stt++,
          asgn.driver.ten,
          asgn.driver.bien_so || '',
      ...
      // Subtotal row
      rows.push(['', `↳ ${asgn.driver.ten}: ${asgn.orders.length} đơn`, '', '', '', '', '', asgn.usedThung, '', '']);
      rows.push([]);
    }

    // Unassigned
    if (suggResult.unassigned && suggResult.unassigned.length > 0) {
      rows.push(['⚠️ CHƯA PHÂN ĐƯỢC', '', '', '', '', '', '', '', '', '']);
      for (const o of suggResult.unassigned) {
        rows.push(['', '', '', o.khu_vuc, o.row_key, o.ten_kh, o.dia_chi_giao, o.thung, '', o.bo_phan]);
      }
    }
    ...
    ws['!cols'] = [
      ...
      { wch: 22 }, { wch: 36 }, { wch: 10 }, { wch: 14 }, { wch: 8 },
    ];
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, 'Phân xe');
    const fileName = `phan-xe-${new Date().toISOString().slice(0,10)}.xlsx`;
...
<script>
  ...
  return (
    <div style={{ paddingBottom: activeDriver ? 80 : 0 }}>
      ...
      {/* ── Modal Gợi ý phân xe ─────────────────────────────────────────── */}
      {suggModal && (
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,.55)', zIndex:3000, display:'flex', alignItems:'flex-start', justifyContent:'center', padding:'24px 12px', overflowY:'auto' }}
          onClick={function(e) { if (e.target === e.currentTarget) setSuggModal(false); }}>
          <div style={{ background:'white', borderRadius:14, width:'100%', maxWidth:820, boxShadow:'0 20px 60px rgba(0,0,0,.25)', position:'relative' }}>

            {/* Header */}
            <div style={{ background:'linear-gradient(135deg,#065f46,#047857)', color:'white', padding:'16px 20px', borderRadius:'14px 14px 0 0', display:'flex', alignItems:'center', gap:10 }}>
              <div style={{ flex:1 }}>
            ...
            <div style={{ padding:'16px 20px', maxHeight:'70vh', overflowY:'auto' }}>
              ...
              {!suggLoading && suggResult && suggResult.error && (
                <div style={{ background:'#fef2f2', border:'1px solid #fca5a5', borderRadius:8, padding:'12px', color:'#dc2626' }}>❌ Lỗi: {suggResult.error}</div>
              )}
              {!suggLoading && suggResult && !suggResult.error && (
                <>
                  {/* Meta stats */}
                  <div style={{ display:'flex', gap:12, marginBottom:16, flexWrap:'wrap' }}>
                    {[
                      ['📦 Tổng đơn cần phân',  suggResult.meta?.total,      '#1d4ed8','#eff6ff','#bfdbfe'],
                      ['✅ Đã gán',              suggResult.meta?.assigned,   '#059669','#ecfdf5','#a7f3d0'],
                      ['⚠️ Chưa gán được',       suggResult.meta?.unassigned, '#d97706','#fffbeb','#fde68a'],
                      ['🚗 Xe sử dụng',          suggResult.meta?.drivers,    '#7c3aed','#f5f3ff','#ddd6fe'],
                    ].map(function([label, val, col, bg, border]) {
                      return (
                        <div key={label} style={{ background:bg, border:`1px solid ${border}`, borderRadius:8, padding:'8px 14px', flex:'1 1 120px', minWidth:110 }}>
                          <div style={{ fontSize:11, color:col, fontWeight:600 }}>{label}</div>
                  ...
                  {/* Kết quả từng xe */}
                  {suggResult.assignments && suggResult.assignments.filter(a => a.orders.length > 0).map(function(asgn) {
                    const pct = asgn.loadPct;
                    const barColor = pct === null ? '#6b7280' : pct >= 90 ? '#dc2626' : pct >= 70 ? '#d97706' : '#059669';
                    const areaList = Object.entries(asgn.areas || {}).map(([a,c]) => `${a} (${c})`).join(', ');
                    return (
                      <div key={asgn.driver.ten} style={{ border:'1px solid #e5e7eb', borderRadius:10, marginBottom:12, overflow:'hidden' }}>
                        <div style={{ background:'#f8fafc', padding:'10px 14px', display:'flex', alignItems:'center', gap:10, borderBottom:'1px solid #e5e7eb' }}>
                          <div style={{ flex:1 }}>
                            <span style={{ fontWeight:700, fontSize:14 }}>🚗 {asgn.driver.ten}</span>
                            {asgn.driver.bien_so && <span style={{ fontSize:11, color:'#6b7280', marginLeft:8 }}>{asgn.driver.bien_so}</span>}
                          </div>
                        ...
                        {areaList && (
                          <div style={{ padding:'6px 14px', fontSize:11, color:'#6b7280', background:'#fafafa', borderBottom:'1px solid #f3f4f6' }}>📍 {areaList}</div>
                        )}
                        <div style={{ padding:'8px 14px' }}>
                          <table style={{ width:'100%', borderCollapse:'collapse', fontSize:12 }}>
                            ...
                            <tbody>
                              {asgn.orders.map(function(o, i) {
                                return (
                                  <tr key={o.row_key} style={{ background: i%2 ? '#f9fafb' : 'white' }}>
                                    <td style={{ padding:'3px 6px 3px 0', fontWeight:500 }}>{o.ten_kh}</td>
                  ...
                  {/* Unassigned */}
                  {suggResult.unassigned && suggResult.unassigned.length > 0 && (
                    <div style={{ border:'1px solid #fde68a', borderRadius:10, overflow:'hidden', marginTop:8 }}>
                      <div style={{ background:'#fffbeb', padding:'8px 14px', fontWeight:700, fontSize:13, color:'#92400e', borderBottom:'1px solid #fde68a' }}>
                        ⚠️ Chưa phân được ({suggResult.unassigned.length} đơn) — vượt sức tải hoặc không có xe
                      </div>
                      <div style={{ padding:'8px 14px' }}>
                  ...
                  {suggResult.info && <div style={{ textAlign:'center', color:'#6b7280', fontSize:13, padding:'20px' }}>{suggResult.info}</div>}
                </>
              )}
            </div>
            ...
            <div style={{ padding:'12px 20px', borderTop:'1px solid #e5e7eb', display:'flex', gap:8, justifyContent:'flex-end', alignItems:'center' }}>
              <span style={{ fontSize:12, color:'#9ca3af', flex:1 }}>Gợi ý tự động — có thể điều chỉnh thủ công sau khi áp dụng.</span>
              <button onClick={exportSuggExcel}
                disabled={!suggResult || !!suggResult.error || !suggResult.meta?.assigned}
                style={{ padding:'8px 16px', border:'1px solid #bfdbfe', borderRadius:8, background:'#eff6ff', color:'#1d4ed8', cursor:'pointer', fontSize:13, fontWeight:600 }}>📥 Xuất Excel</button>
              <button onClick={() => setSuggModal(false)}
                style={{ padding:'8px 16px', border:'1px solid #d1d5db', borderRadius:8, background:'white', cursor:'pointer', fontSize:13 }}>Huỷ</button>
              <button
                onClick={applySuggestion}
                disabled={suggApplying || suggLoading || !suggResult || !!suggResult.error || !suggResult.meta?.assigned}
                style={{ padding:'8px 20px', border:'none', borderRadius:8, background: suggApplying ? '#a7f3d0' : '#059669', color:'white', cursor:'pointer', fontSize:13, fontWeight:700 }}>
                {suggApplying ? '⏳ Đang áp dụng…' : `✅ Áp dụng (${suggResult?.meta?.assigned || 0} đơn)`}
              </button>
            </div>
          </div>
        </div>
