...
function doPost(e) {
  try {
    ...
    var result = writeBack(cfg.id, soPhieu, laiXe, giaoNhan, ngayGiao);
...
// ── Hàm ghi ngược ────────────────────────────────────────────────────────────
function writeBack(sheetId, soPhieu, laiXe, giaoNhan, ngayGiao) {
  var ss = SpreadsheetApp.openById(sheetId);

  // Tìm tab theo tháng (hoặc tìm trong tất cả tab)
  var sheet = findSheet(ss, ngayGiao);
  ...
  for (var i = 0; i < numRows; i++) {
    ...
    if (rowNorm.some(function(h) { return h.indexOf('SO PHIEU') >= 0 || h.indexOf('MA LENH') >= 0; })) {
      headerRow = i;
      break;
    }
  }
  ...
  // Tìm column indices
  var soPhieuCol  = findColIdx(headers, function(h) { return h.indexOf('SO PHIEU') >= 0; });
  var maLenhCol   = findColIdx(headers, function(h) { return h.indexOf('MA LENH') >= 0; });
