...
function parseRows(rows, boPhan) {
  // Tìm hàng header chứa 'SỐ PHIẾU' hoặc 'MÃ LỆNH' (MT dùng cả hai)
  let headerIdx = -1;
  for (let i = 0; i < rows.length; i++) {
    if (rows[i].some(c => norm(c).includes('SỐ PHIẾU') || norm(c).includes('MÃ LỆNH'))) { headerIdx = i; break; }
  }
  ...
  for (let i = headerIdx + 1; i < rows.length; i++) {
    ...
    // SỐ PHIẾU là key duy nhất cho từng dòng (1 mã lệnh có thể có nhiều phiếu)
