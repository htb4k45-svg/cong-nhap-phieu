...
async function fetchSheet(sheetId, sheetName) {
  ...
  try {
    ...
    // Sheet trống hoặc không đúng format → bỏ qua
    const firstLine = text.split('\n')[0] || '';
    if (!firstLine.includes('PHIEU') && !firstLine.includes('LENH') && !firstLine.includes('KHACH')) return null;
    return text;
  } catch {
    return null;
  }
...
export async function fetchPhieuByDateRange(fromDate, toDate) {
  ...
  // Lọc theo khoảng ngày giao yêu cầu
  // Đơn không có ngày → giữ lại (cần phân xe)
  // Đơn đã được app ghi NGÀY GIAO thực tế → vẫn hiển thị (để biết đã xong)
